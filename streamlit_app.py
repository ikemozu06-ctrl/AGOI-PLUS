...
def data_badge(meta: dict):
    """
    Headline data-status banner. Kept clean and user-facing: the technical
    provenance details (run ID, AfDB connector status) live in a collapsed
    expander so the page looks finished while staying fully auditable.
    """
    mode = meta["data_mode"]
    if mode == "live":
        st.success(f"🟢 **Live World Bank data** · {meta['n_countries']} countries · "
                   f"{meta['n_indicators']} indicators")
    elif mode == "mix":
        st.info(f"🔵 **Live World Bank data** · {meta['n_countries']} countries · "
                f"{meta['n_indicators']} indicators "
                "(any gaps filled with clearly-flagged sample values)")
    else:
        st.warning("🟡 **DEMO DATA — not real measurements.** "
                   "Synthetic sample for demonstration; all values flagged low-confidence. "
                   "Switch to Live/Mixed in the sidebar for real World Bank data.")
